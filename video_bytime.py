...
expectd_time = int(sys.argv[1])  #接受输入参数
expectd_min = expectd_time - int(sys.argv[2]) #接受最小范围
expectd_max = expectd_time + int(sys.argv[3]) #接受最大范围
...
def save_list_result():
    with open("result/time_result.txt","w") as f:
        f.truncate()
